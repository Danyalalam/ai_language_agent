...
It also includes a live German speaking agent powered by Azure VoiceLive for real-time conversation practice.
...
Run the live voice agent from the repository root after setting these variables:
...
VOICELIVE_VOICE=en-US-AvaNeural
VOICELIVE_INSTRUCTIONS=You are a helpful AI assistant.
```

Optional if you want token-based auth instead of an API key:
